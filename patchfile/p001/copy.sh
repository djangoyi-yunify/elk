...
lst_copy() {
  ...
  if [ ! -f $CUR_PATH/back/log4j-core-2.11.1.jar ]; then
    cp /usr/share/logstash/logstash-core/lib/jars/log4j-core-2.11.1.jar $CUR_PATH/back
  fi

  # copy
  cp -f $CUR_PATH/etc/confd/templates/logstash.sh.tmpl /etc/confd/templates/logstash.sh.tmpl
  cp -f $CUR_PATH/usr/share/logstash/logstash-core/lib/jars/log4j-core-2.11.1.jar /usr/share/logstash/logstash-core/lib/jars/log4j-core-2.11.1.jar
}

lst_info() {
  ls -l /etc/confd/templates/logstash.sh.tmpl
  cat /etc/confd/templates/logstash.sh.tmpl | grep log4j
  ls -l /usr/share/logstash/logstash-core/lib/jars/log4j-core-2.11.1.jar
  if [ -f $CUR_PATH/back/log4j-core-2.11.1.jar ]; then
    echo "*****backup jar info*****"
    /usr/bin/jar -tf $CUR_PATH/back/log4j-core-2.11.1.jar | grep -i jndi
  fi
  echo "*****sys jar info*****"
